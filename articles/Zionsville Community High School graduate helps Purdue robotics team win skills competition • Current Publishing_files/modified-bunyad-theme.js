...
var Bunyad_Theme = (function($) {
	...
	return {
      ...
      // setup element menu
      this.responsive_nav_menu_position_ready();
      this.responsive_nav_menu_position_resize();
      this.responsive_nav_sub_menu_position_ready();
      this.responsive_nav_sub_menu_position_resize();
            ...
            $('.main img, .main-footer img, .main-featured .row').each(function () {
    ...
    responsive_nav_menu_position: function () {
      let offsetDocWidth = document.documentElement.offsetWidth;
      let elements = document.querySelectorAll('#menu-categories > li > ul');

      elements.forEach((el) => {
        let elCoordinates = el.getBoundingClientRect();
        let elOuterWidth = el.offsetWidth;
        let parentElement = el.parentElement.getBoundingClientRect().left;

        if (elCoordinates.left + elOuterWidth > offsetDocWidth) {
          if (!el.classList.contains('adaptive-menu-position')) {
            el.classList.add('adaptive-menu-position');
          }
        }

        if ((parentElement + elOuterWidth < offsetDocWidth) && el.classList.contains('adaptive-menu-position')) {
          el.classList.remove('adaptive-menu-position');
        }
      });
    },

    // responsive navigation menu position
    responsive_nav_menu_position_ready: function () {
      $(window).ready(() => this.responsive_nav_menu_position());
    },

    responsive_nav_menu_position_resize: function () {
      $(window).resize(() => this.responsive_nav_menu_position());
    },

    responsive_nav_sub_menu_position: function () {
      let offsetDocWidth = document.documentElement.offsetWidth;
      let elements = document.querySelectorAll('#menu-categories ul > li > ul');
      ...
      elements.forEach((el) => {
        ...
        if (elCoordinates.left + elOuterWidth > offsetDocWidth) {
          if (!el.classList.contains('adaptive-sub-menu-position')) {
            el.classList.add('adaptive-sub-menu-position');
          }
        }

        if (elCoordinates.left <= 0 && el.classList.contains('adaptive-sub-menu-position')) {
          el.classList.remove('adaptive-sub-menu-position');
        }

      });
    },

    // responsive navigation sub-menu position
    responsive_nav_sub_menu_position_ready: function () {
      $(window).ready(() => this.responsive_nav_sub_menu_position());
    },

    responsive_nav_sub_menu_position_resize: function () {
      $(window).resize(() => this.responsive_nav_sub_menu_position());
    },
